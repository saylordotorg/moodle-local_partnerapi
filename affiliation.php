...
if ($join && confirm_sesskey()) {
    $cohort = local_partnerapi_get_self_service_affiliation($join);
    if (!$cohort) {
        redirect(
            $thisurl,
            get_string('affiliationinvalid', 'local_partnerapi'),
            null,
    ...
    redirect(
        $thisurl,
        get_string('affiliationjoined', 'local_partnerapi', format_string($cohort->name)),
        null,
...
if ($leave && confirm_sesskey()) {
    ...
    $isaffiliation = $cohort &&
        stripos((string) $cohort->idnumber, LOCAL_PARTNERAPI_AFFILIATION_PREFIX) === 0;
    if (
        $isaffiliation &&
        ...
        redirect(
            $thisurl,
            get_string('affiliationremoved', 'local_partnerapi', format_string($cohort->name)),
            null,
            \core\output\notification::NOTIFY_SUCCESS
        );
    }
...
echo html_writer::tag(
    'p',
    get_string('affiliation_intro', 'local_partnerapi'),
...
if (!empty($current)) {
    ...
    $aff = reset($current); // Only one is allowed.
    ...
    echo html_writer::tag(
        ...
        html_writer::tag(
            'span',
            ' — ' . get_string('youraffiliations', 'local_partnerapi'),
            ['class' => 'text-muted']
        ),
    );
    ...
    echo html_writer::link(
        $leaveurl,
        get_string('affiliationleave', 'local_partnerapi'),
